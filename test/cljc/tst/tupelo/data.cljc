         ...
         (with-tdb (new-tdb)
           ...
           (let [triple-specs [[(tag-param :x) :a 1]]]
             (let [pred1 (fn [query-result]
               ...
               (is= (td/tagval-walk-compact (query-triples triple-specs))
                 [{{:param :x} {:eid 1001}}])
         ...
         (dotest
           (with-tdb (new-tdb)
             ...
             (let [edn-val  {:a 1
               ...
               (is= (td/tagval-walk-compact (deref *tdb*))
                 {:eid-type {{:eid 1001} :map},
                  :idx-ave  #{[:b 1 {:eid 1001}] [:a 1 {:eid 1001}]},
                  :idx-eav  #{[{:eid 1001} :a 1] [{:eid 1001} :b 1]},
                  :idx-vae  #{[1 :b {:eid 1001}] [1 :a {:eid 1001}]}}))
       ...
       (dotest
         (td/with-tdb (td/new-tdb)
           ...
           (let [root-hid (td/add-edn edn-val)]
             ...
             (is= (td/unwrap-query-results [{(tag-param :a) 1}])
               [{:a 1}])
             (is= (td/unwrap-query-results [{(tag-param :x) (tag-eid 1001),
                                             (tag-param :y) (tag-eid 1002),
                                             (tag-param :a) 1}])
               [{:x 1001, :y 1002, :a 1}])
             (is= (td/unwrap-query-results [{(tag-param :e) (tag-eid 1003)
                                             (tag-param :i) 2}])
               [{:e 1003, :i 2}])
